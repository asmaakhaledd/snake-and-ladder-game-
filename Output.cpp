...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	int R;
	R = ((cellPos.VCell()) * UI.CellHeight)+ UI.ToolBarHeight;
	return R;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_OBJECT] ="images\\Menu_Killer.jpg";	 
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_VALUE] = "images\\AddValue.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	SS.SetHCell(LL);
	int CellStartYY;
	for (int q=toCell.VCell()+1; q<=fromCell.VCell; q++)
	{
	SS.SetVCell(q);
	CellStartYY=GetCellStartY(SS);
	pWind->DrawLine(x12,CellStartYY, x34, CellStartYY);8*/
	int h = fromStartY;
	for(int i = 0; i < (fromCell.VCell() - toCell.VCell()); ++i){			
		pWind->DrawLine(x12, h, x34, h);
		h -= UI.CellHeight;
	}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	pWind->SetPen(RED, UI.SnakelineWidth);

	///TODO: Draw the Line representing the Snake BOdy
	pWind->DrawLine(x12, y1, (x12 - 1), (y2 - 1), FRAME);
	...
	int x_l = x12 - xChange;
	int x_r = x12 + xChange;
	int y_l_r = y1 + yChange;
	int y2_s = y1 + 2 * yChange;
	int A_X[4];
	int A_Y[4];
	A_X[0] = x12;
	A_X[1] = x_r;
